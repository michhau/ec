...
orthomosaic_makie = permutedims(reverse(orthomosaic_jl, dims=1), (2, 1))
image_x = LinRange(bgextend_final[1], bgextend_final[2], size(orthomosaic_makie, 1))
image_y = LinRange(bgextend_final[3], bgextend_final[4], size(orthomosaic_makie, 2))

ffp_fig = CairoMakie.Figure(size=(700, 500))
ax1 = CairoMakie.Axis(ffp_fig[1, 1]; xlabel="meter", ylabel="meter", aspect=CairoMakie.DataAspect())
#ax1.title = "Station $(station_label) Flux footprints"
bg = CairoMakie.image!(ax1, image_x, image_y, orthomosaic_makie)
locfx1 = CairoMakie.scatter!(ax1, [fluxloc_final[1, 2]], [fluxloc_final[1, 1]]; color=makie_tab10[1], markersize=15)
#locfx2 = CairoMakie.scatter!(ax1, [fluxloc_final[2, 2]], [fluxloc_final[2, 1]]; color=makie_tab10[2], markersize=15)
locfx3 = CairoMakie.scatter!(ax1, [fluxloc_final[3, 2]], [fluxloc_final[3, 1]]; color=makie_tab10[3], markersize=15)
#locfx4 = CairoMakie.scatter!(ax1, [fluxloc_final[4, 2]], [fluxloc_final[4, 1]]; color=makie_tab10[4], markersize=15)
fp1 = CairoMakie.lines!(ax1, footprint_contour(ffp1["xr"], contour_indices[1]) .+ fluxloc_final[1, 2], footprint_contour(ffp1["yr"], contour_indices[1]) .+ fluxloc_final[1, 1]; color=makie_tab10[1], label=instr_labels[1])
fp2 = CairoMakie.lines!(ax1, footprint_contour(ffp2["xr"], contour_indices[2]) .+ fluxloc_final[2, 2], footprint_contour(ffp2["yr"], contour_indices[2]) .+ fluxloc_final[2, 1]; color=makie_tab10[2], label=instr_labels[2])
fp3 = CairoMakie.lines!(ax1, footprint_contour(ffp3["xr"], contour_indices[3]) .+ fluxloc_final[3, 2], footprint_contour(ffp3["yr"], contour_indices[3]) .+ fluxloc_final[3, 1]; color=makie_tab10[3], label=instr_labels[3])
fp4 = CairoMakie.lines!(ax1, footprint_contour(ffp4["xr"], contour_indices[4]) .+ fluxloc_final[4, 2], footprint_contour(ffp4["yr"], contour_indices[4]) .+ fluxloc_final[4, 1]; color=makie_tab10[4], label=instr_labels[4])
...
=#
###################################################################################################
#Figures for pond section (Figs. xxx)
...
station_name = "1b_2" # Change this to "1a", "1b_1", ..., "3d" before including the script.
...
axs_diff_scatter[1].set_xlim(0.3, 0.7)
axs_diff_scatter[2].set_xlim(0.6, 0.75)
axs_diff_scatter[3].set_xlim(0.6, 0.75)
